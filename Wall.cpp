...
void Wall::getImgSet(std::string name)
{
    placedImgSet = &(*rendering->findImageSet("Tiles_wall")->find(name)).second;
    placedImgSize.x = 34;
    placedImgSize.y = 34;
    itemImgSize.x = 34;
    itemImgSize.y = 34;
...
void Wall::changeBlockImg(std::string state, int shadow)
{
    std::string imgInfo;
...
Vector2Int Wall::imageFinder(std::string state)
{
    if (imgInfo.empty())
    {
        imgInfo = {
